  ...
  bool searchMatrix(vector<vector<int>>& matrix, int target) {
       if(!matrix.size())return false;
        int n = matrix.size(),
         ...
         while(i<n && j>=0){   // keep on moving till it does not get out of bound 
             if(matrix[i][j]==target){
                 return true;
             }
             if(matrix[i][j]>target){
  ...
  bool searchMatrix(vector<vector<int>>& matrix, int target) {
       if(!matrix.size())return false; // if matrix is empty 
        int n = matrix.size(), // numnber of row
           ...
           int l = 0;  
           int h = (n*m)-1;  
           while(l<=h){   // perform binary search
               int mid = l+(h-l)/2;  // to avoid overflow 
               if(matrix[mid/m][mid%m]==target){ // row index for imaginary mid index = mid/m  , col = mid%m
                   return true;
               }
               if(matrix[mid/m][mid%m]>target){ // if target is lesser search on the left half
                   h = mid-1;
               }
               else{    // if target is greater earch on the right half
                   l = mid+1;
               }
           }
           return false;
